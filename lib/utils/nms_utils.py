...
def cpu_cam_nms(dets, base_thr, high_thr, low_thr):
    """Pure Python NMS baseline."""
    dets = dets.numpy()
    x1 = dets[:, 0]
    y1 = dets[:, 1]
    ...
    while len(order) > 0:
        ...
        sim = np.sum(cams[i] * cams[order[1:]], axis=1)

        reserve_cond1 = (ovr <= base_thr) * (sim < high_thr)
        reserve_cond2 = (ovr > base_thr) * (sim < low_thr)

        inds = np.where(reserve_cond1 + reserve_cond2)[0]
        order = order[inds + 1]
    return np.array(keep)

def cpu_feature_nms(dets, high_thr, low_thr):
    """Pure Python Feature NMS."""
    dets = dets.numpy()
    x1 = dets[:, 0]
    y1 = dets[:, 1]
    x2 = dets[:, 2]
    ...
    while len(order) > 0:
        ...
        dists = np.linalg.norm(
            features[i] - features[order[1:]],
            ord=2,
            axis=1)
        reserve_cond1 = (ovr < low_thr)
        reserve_cond2 = (ovr < high_thr) * (dists > 1.0)
