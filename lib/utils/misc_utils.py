...
def load_coco_lines(coco_api, is_train):
    records = []
    ...
    if is_train:
        for img, anno in list(zip(imgs, anns)):
            if has_valid_annotation(anno):
                records.append((img, anno))
    records = list(zip(imgs, anns))  
    return records
...
def load_coco_gt(record_boxes, id_map):
    # [{'segmentation': [[192.81,
    #     247.09,
    #     ...
    #     219.03,
    ...
    #   'id': 42986},
    #  ...]
    if len(record_boxes) < 1:
        return np.empty([0, 5])
    bbox = []
    for rb in record_boxes:
        if rb['iscrowd'] == 1:
            tag = -1
        else:
            tag = id_map[rb['category_id']]
        bbox.append(np.hstack((rb['bbox'], tag)))
    bboxes = np.vstack(bbox).astype(np.float64)
    return bboxes
