...
def fpn_roi_target(config, rpn_rois, im_info, gt_boxes, top_k=1):
    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        return_gt_assignments.append(gt_assignment)
    if config.train_batch_per_gpu == 1:
        return rois, labels, bbox_targets, gt_assignment
    else:
        ...
        return_gt_assignments = torch.cat(return_gt_assignments, axis=0)
        return return_rois, return_labels, return_bbox_targets, return_gt_assignments
