...
def paired_box_overlap_opr(box1, box2):
    # Nx4, Nx4 --> N 
    assert box1.ndim == 2
    assert box2.ndim == 2
    area1 = (box1[:, 2] - box1[:, 0] + 1) * (box1[:, 3] - box1[:, 1] + 1)
    area2 = (box2[:, 2] - box2[:, 0] + 1) * (box2[:, 3] - box2[:, 1] + 1)
    width_height = torch.min(box1[:,2:], box2[:, 2:]) - torch.max(
        box1[:, :2], box2[:, :2]) + 1  # [N,2]
...
def paired_box_overlap_opr_diou(box1, box2):
    # Nx4, Nx4 --> N 
    assert box1.ndim == 2
    assert box2.ndim == 2
    area1 = (box1[:, 2] - box1[:, 0] + 1) * (box1[:, 3] - box1[:, 1] + 1)
    ...
    c_box1 = (box1[:, 2:4] + box1[:, 0:2])/2
    c_box2 = (box2[:, 2:4] + box2[:, 0:2])/2
    most_left = torch.min(box1[:, :2], box2[:, :2])
    most_right = torch.max(box1[:, 2:], box2[:, 2:])
    d = ((c_box2-c_box1)**2).sum(-1) / ((most_right-most_left)**2).sum(-1)
