...
def fpn_rpn_reshape(config, pred_cls_score_list, pred_bbox_offsets_list):
...
def fpn_anchor_target_opr_core_impl(
        config, gt_boxes, im_info, anchors, allow_low_quality_matches=True):
...
@torch.no_grad()
def fpn_anchor_target(config, boxes, im_info, all_anchors_list):
    final_labels_list = []
    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        for i in range(len(all_anchors_list)):
            ...
            rpn_labels_perlvl, rpn_bbox_targets_perlvl = fpn_anchor_target_opr_core_impl(
                config, boxes[bid], im_info[bid], anchors_perlvl)
        ...
        # sample labels
        pos_idx, neg_idx = subsample_labels(config, concated_batch_labels)
...
def subsample_labels(config, labels):
    num_samples, positive_fraction = config.num_sample_anchors, config.positive_anchor_ratio
