...
def add_pixel(image, input_img, poses):
    xs, ys = zip(*poses)
    input_img[0,:, ys, xs] = image[0,:, ys, xs]
    return input_img


def insert_process(net, image, im_info, heatmap, L, grids, gt_box, gt_class, img_id, gt_id, path):
    ...
    input_img = torch.zeros(image.size())
    scores = []
    for step in range(1,L+1):
        input_img = add_pixel(image, input_img, grids[order[(step-1)*pixel_once:step*pixel_once]])
        if step%10 == 0:
            pred_boxes, _, _ = \
                net(input_img.clone().cuda(), im_info.clone().cuda())
    ...
    input_img = input_img.squeeze(0).permute(1,2,0).numpy().astype(np.uint8)
    # image = cv2.cvtColor(image,cv2.COLOR_RGB2BGR) 
    cv2.imwrite(path+'{}_{}.jpg'.format(img_id, gt_id), input_img)
    return scores
...
def inference(config, network, model_file, dataset, start, end, objs_list, map_path):
    ...
    path = './insert_visual/{}/'.format(method_tag)
    ...
    for (image, im_info, ID, anno, image_ori) in data_iter:
        if int(ID) in objs_list.keys():
            ...
            for gt_id in objects:
                ...
                scores_1toL = insert_process(net, image.clone(), im_info.clone(), \
                    heatmap, L, grids, gt_boxes[gt_id], gt_classes[gt_id], int(ID), gt_id, path)
                
                add_L = (torch.tensor([score_0]+scores_1toL))
                add_cums.append(add_L)
        
            add_cums_tmp = torch.mean(torch.stack(add_cums,dim=0), dim=0)   

            print(' ')
            print('add_cums_tmp:', add_cums_tmp)
        pbar.update(1)
    
    print('number object counted:', len(add_cums))
    add_cums = torch.mean(torch.stack(add_cums,dim=0), dim=0)   
    
    print('map path', map_path)
    print('add_cums:', add_cums)
